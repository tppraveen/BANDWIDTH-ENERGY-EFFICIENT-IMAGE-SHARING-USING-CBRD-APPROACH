...
public class FloatArray2DSIFT 
{
	 ...
	 * octaved scale space
	 */
	private FloatArray2DOctave[] octaves;
	public FloatArray2DOctave[] getOctaves()
	{
		return octaves;
	}
	public FloatArray2DOctave getOctave( int i )
	{
		return octaves[ i ];
	}
			...
			int max_size )
	{
		...
		octaves = new FloatArray2DOctave[ o ];
		...
		{
			octaves[ i ] = new FloatArray2DOctave(
					src,
					sigma,
					sigma_diff,
					kernel_diff );
			...
			FloatArray2DOctave.downsample( octaves[ i ].getL( 1 ), next );
	...
	private float[] createDescriptor(
			...
			float orientation )
	{
		FloatArray2DOctave octave = octaves[ o ];
	...
	public Vector< Feature > run()
	{
		...
		{
			if ( octaves[ o ].state == FloatArray2DOctave.State.EMPTY ) continue;
